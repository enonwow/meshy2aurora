...
## 0. Kanoniczna rodzina worktree — HARD STOP

Kanonicznym repozytorium jest `C:\Projects\meshy2aurora`. Jego glowny worktree
oraz wszystkie zarejestrowane przez nie linked worktree sa zapisywalnymi
workspace projektu. Rownolegle branche powinny pracowac w osobnych worktree.
...
common directory. Dozwolony jest root widoczny w `git worktree list`, ktorego
common directory wskazuje na `C:\Projects\meshy2aurora\.git`. Inny klon lub
niezarejestrowany katalog wymaga HARD STOP. Pelny kontrakt znajduje sie w
...
Foldery o podobnej nazwie, ktore nie sa linked worktree zarejestrowanym przez
kanoniczne repozytorium, nie sa kanoniczne dla tego projektu.
