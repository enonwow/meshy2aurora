...
use m2a_core::{
    ...
    item_material_mapping::{
        ...
        UnmappedSourceMaterialPolicyV1, build_item_material_plt_v1,
...
const SOURCE_SHA256: &str = "e21998acceb551e756c789b053059e2880bc847fb7fbc17c3cc12aed3ee297bc";
...
const VARIANT: u8 = 224;
const MODEL_RESREF: &str = "helm_224";
const CLOTH_TEXTURE_RESREF: &str = "h224cloth";
const METAL_TEXTURE_RESREF: &str = "h224metal";
const ICON_RESREF: &str = "ihelm_224";
const UTI_RESREF: &str = "m2bronmask224";
const HAK_RESREF: &str = "m2bronh224v1";
const MODULE_RESREF: &str = "m2bronpal224";
const MODULE_FILENAME: &str = "m2bronpal224.mod";
const MODULE_DISPLAY_NAME: &str = "Meshy2Aurora Bronze Mask Split Palette 224";
const AREA_DISPLAY_NAME: &str = "Bronze Mask Split Item Palette 224";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let source_path = root.join(
        "sample-3d/tlc-bronze-mask-tripo-20260904-v1/source-material-split-static-clean-v1.glb",
    );
    let icon_source_path = root.join(
        "artifacts/diagnostics/tlc-bronze-mask-tripo-20260904-material-split-v1-offline-qa/renders/render-front.png",
    );
    ...
    let output_root = env::var("M2A_BRONZE_MASK_224_OUTPUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            root.join("proof-output/tlc-bronze-mask-split-helmet-palette-only-224-v1-20260904")
        });
    ...
    if !conversion.report.conversion_eligible {
        return Err(format!(
            "Bronze Mask 224 source failed exact static Item Profile A admission: {}",
    ...
    let recipe = ItemAppearanceRecipeV1 {
        ...
        identity: ItemIdentityV1 {
            ...
            tag: "M2A_BRONZE_MASK_224_V1".to_owned(),
            display_name: "Brązowa Maska 224".to_owned(),
        },
        gender: None,
        parts: vec![ItemPartRecipeV1 {
            ...
            variant: VARIANT,
            source_part_id:
                "tlc-bronze-mask-tripo-20260904-v1/source-material-split-static-clean-v1.glb"
                    .to_owned(),
    ...
    {
        return Err("helmet naming did not resolve to the exact variant-224 namespace".into());
    }
    let recipe_sha256 = item_recipe_sha256_v1(&recipe)?;

    let material_policy = ItemMaterialLayerMappingPolicyV1 {
        schema_version: 1,
        mappings: vec![
    ...
    let texture_selections = resolve_base_color_image_indices_v1(&ingest, model)?;
    if texture_selections.len() != 2
        || !texture_selections
            .iter()
            .any(|selection| selection.source_material_id == 0)
        || !texture_selections
            .iter()
            .any(|selection| selection.source_material_id == 1)
    {
        return Err("Bronze Mask 224 does not expose the exact two source materials".into());
    }
    let mut material_bindings = Vec::with_capacity(texture_selections.len());
    let mut model_plts = Vec::with_capacity(texture_selections.len());
    for selection in &texture_selections {
        let explicit_layer =
            resolve_item_source_material_layer_v1(&material_policy, selection.source_material_id)?;
        let (texture_resref, expected_layer) = match selection.source_material_id {
            0 => (CLOTH_TEXTURE_RESREF, NwnItemPaletteLayerV1::Cloth1),
            1 => (METAL_TEXTURE_RESREF, NwnItemPaletteLayerV1::Metal1),
            other => return Err(format!("unexpected source material id {other}").into()),
        };
        if explicit_layer != Some(expected_layer) {
            return Err(format!(
                "source material {} did not resolve to its explicit NWN layer",
                selection.source_material_id
            )
            .into());
        }
        ...
        let plt_image = build_item_material_plt_v1(&diffuse_image, explicit_layer)?;
        let layer_counts = layer_counts(&plt_image);
        require_uniform_material_layer(&layer_counts, expected_layer, texture_resref)?;
        let plt = write_plt_v1(&plt_image, &PltWriterOptionsV1::default())?;
        material_bindings.push(MdlMaterialTextureBindingV1 {
    ...
    material_bindings.sort_by_key(|binding| binding.material_slot);
    model_plts.sort_by_key(|entry| entry.1);
    ...
    let compiled = compile_meshy_static_item_part_v1(
        ...
        &MeshyItemPartCompileRequestV1 {
            ...
            material_textures: material_bindings.clone(),
        },
    )?;
    ...
    {
        return Err("Bronze Mask 224 MDL did not preserve the sanitized geometry, class and stream partitioning".into());
    }
    let mut compiled_texture_resrefs = Vec::new();
    for root_node in &compiled.inspection.node_tree.roots {
        collect_mesh_texture_resrefs(root_node, &mut compiled_texture_resrefs);
    }
    compiled_texture_resrefs.sort();
    compiled_texture_resrefs.dedup();
    if compiled_texture_resrefs != [CLOTH_TEXTURE_RESREF, METAL_TEXTURE_RESREF] {
        return Err(format!(
            "Bronze Mask 224 MDL texture readback differs: {compiled_texture_resrefs:?}"
        )
        .into());
    }
    ...
    if transparent_icon_pixels == 0 || !icon_border_is_transparent(&icon_plt_image) {
        return Err("Bronze Mask 224 icon PLT has no exact transparent background border".into());
    }
    ...
    let module_fixture_resources = build_helmet_224_module_fixture(&source_module)?;
    let package = write_item_package_v1(
        &ItemPackageBuildRequestV1 {
            schema_version: ITEM_SCHEMA_VERSION,
            generator_identity: "materialize_tlc_bronze_mask_split_helmet_224_v1".to_owned(),
            recipe: recipe.clone(),
            namespace,
            uti: ItemUtiBuildRequestV1 {
                ...
                properties: ItemUtiPropertiesV1 {
                    ...
                    identified_description:
                        "Wariant 224: źródłowy materiał 0 mapuje się do Cloth 1, materiał 1 do Metal 1; dopasowanie jak w wariancie 222."
                            .to_owned(),
                    cost: 1,
                    comment: "TLC Bronze Mask split-material palette-only candidate 224 V1."
                        .to_owned(),
                    palette_id: 9,
            ...
            icons: icons.clone(),
            additional_hak_resources: model_plts
                .iter()
                .map(|entry| HakResourceInputV1 {
                    resref: entry.2.to_owned(),
                    resource_type: PLT_RESOURCE_TYPE,
                    payload: entry.6.payload.clone(),
                })
                .collect(),
    ...
    {
        return Err(
            "Bronze Mask 224 package is not the exact palette-only ordered-HAK fixture".into(),
        );
    }
    ...
    let offline_report = serde_json::to_vec_pretty(&json!({
        ...
        "lineageDecision": {
            "previousHelm223V2": "owner_rejected_exact_m2bronpal223_mod_as_much_worse_than_previous",
            "minimalDelta": [
                "replace single-material source with owner-supplied two-material source",
                "restore donor-top-fit transform policy used by helm_222",
                "map source material 0 explicitly to Cloth 1",
                "map source material 1 explicitly to Metal 1"
            ],
            "relationship": "fresh_owner_requested_source_and_material_mapping_correction",
        },
        "auroraDecompContract": {
            ...
            "explicitSourceMaterialMappingPrecedesAutomaticInference": true,
            "unmappedSourceMaterialPolicy": "AUTOMATIC_CLOTH_METAL",
        },
        "candidate": {
        ...
        "source": {
            ...
            "ownerSourceSha256": OWNER_SOURCE_SHA256,
            "byteLength": source_glb.len(),
            "triangleCount": SOURCE_TRIANGLES,
            "sanitizedTriangleCount": COMPILED_TRIANGLES,
            "removedDegenerateTriangleCount": 2,
            "meshObjectCount": 2,
            "primitiveCount": 2,
            "materialCount": 2,
            "materialAssignments": [
                {"sourceMaterialId": 0, "sourceNode": "tripo_part_0", "targetLayer": "CLOTH_1", "targetLayerIndex": 4},
                {"sourceMaterialId": 1, "sourceNode": "tripo_part_11", "targetLayer": "METAL_1", "targetLayerIndex": 2}
            ],
        },
        "fit": {
            ...
            "placementPolicy": "RESTORED_HELM_222_DONOR_TOP_FIT",
        ...
        "modelPlts": model_plts.iter().map(|entry| json!({
            "sourceMaterialId": entry.0,
            "materialSlot": entry.1,
            "textureResref": entry.2,
            "targetLayer": entry.3,
            "targetLayerIndex": entry.3.layer_index(),
            "sha256": entry.6.report.output_sha256,
            "width": entry.4.width,
            "height": entry.4.height,
            "numLayers": entry.4.num_layers,
            "usedLayerPixelCounts": entry.5,
            "mappingMode": "EXPLICIT_SOURCE_MATERIAL",
        })).collect::<Vec<_>>(),
        "iconPlt": {
            ...
            "materialIdMaskSha256": sha256_hex(&fs::read(&icon_material_id_path)?),
            "mappingMode": "EXPLICIT_RENDERED_SOURCE_MATERIAL_ID",
        },
        "defaultColors": {
            "metal1": DEFAULT_METAL1_ROW,
            "cloth1": DEFAULT_CLOTH1_ROW,
    ...
    for entry in &model_plts {
        write_new(
            &output_root.join(format!("{}.plt", entry.2)),
            &entry.6.payload,
        )?;
    }
    write_new(
...
fn collect_mesh_texture_resrefs<'a>(
    ...
    for child in &node.children {
        collect_mesh_texture_resrefs(child, output);
    }
}
    ...
    for (rgba, material_id) in resized.pixels().zip(resized_material_ids.pixels()) {
        if rgba[3] < 64 {
            ...
            let layer_index = if material_id[0] > 64 && material_id[1] > 64 {
                NwnItemPaletteLayerV1::Metal1.layer_index()
            } else {
                NwnItemPaletteLayerV1::Cloth1.layer_index()
            };
...
fn require_uniform_material_layer(
    counts: &[usize; 10],
    expected_layer: NwnItemPaletteLayerV1,
    role: &str,
) -> Result<(), Box<dyn std::error::Error>> {
    let expected_index = usize::from(expected_layer.layer_index());
    let expected_count = counts[expected_index];
    let unexpected_count = counts
        .iter()
        .enumerate()
        .filter(|(index, _)| *index != expected_index)
        .map(|(_, count)| *count)
        .sum::<usize>();
    if expected_count == 0 || unexpected_count != 0 {
        return Err(format!(
            "{role} PLT is not uniformly assigned to layer {expected_index}: expected={expected_count}, unexpected={unexpected_count}"
        )
        .into());
    }
    Ok(())
...
fn build_helmet_224_module_fixture(
    source_module: &[u8],
        ...
        match descriptor.resource_type {
            IFO_RESOURCE_TYPE => {
                ...
                replace_loc_string(
                    &mut document.root,
                    "Mod_Description",
                    "Palette-only Bronze Mask 224 module: two authored Tripo materials; source material 0 maps to Cloth 1, source material 1 maps to Metal 1, with helm_222 donor-top-fit and 36365 sanitized triangles.",
                )?;
...
fn validate_package_readback(
    ...
    let cloth_plt = read_plt_image_v1(hak.find(CLOTH_TEXTURE_RESREF, PLT_RESOURCE_TYPE)?)?;
    let metal_plt = read_plt_image_v1(hak.find(METAL_TEXTURE_RESREF, PLT_RESOURCE_TYPE)?)?;
    let icon_plt = read_plt_image_v1(hak.find(ICON_RESREF, ITEM_ICON_PLT_RESOURCE_TYPE)?)?;
    if cloth_plt.num_layers != 5 || metal_plt.num_layers != 5 || icon_plt.num_layers != 5 {
        return Err("HAK PLT readback lost the exact layer contract".into());
    }
    require_uniform_material_layer(
        &layer_counts(&cloth_plt),
        NwnItemPaletteLayerV1::Cloth1,
        CLOTH_TEXTURE_RESREF,
    )?;
    require_uniform_material_layer(
        &layer_counts(&metal_plt),
        NwnItemPaletteLayerV1::Metal1,
        METAL_TEXTURE_RESREF,
    )?;
    if hak.find(CLOTH_TEXTURE_RESREF, 3).is_ok()
        || hak.find(METAL_TEXTURE_RESREF, 3).is_ok()
        || hak.find(ICON_RESREF, 3).is_ok()
    {
        return Err(
            "Bronze Mask 224 HAK contains forbidden fallback TGA for a ModelType-1 helmet".into(),
        );
    }
    ...
    {
        return Err(
            "Bronze Mask 224 UTI readback lost variant 224 or the default PLT color rows".into(),
        );
    }
    Ok(())
}
