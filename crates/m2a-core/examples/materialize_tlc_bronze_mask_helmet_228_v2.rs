...
const VARIANT: u8 = 228;
const MODEL_RESREF: &str = "helm_228";
const ICON_RESREF: &str = "ihelm_228";
const UTI_RESREF: &str = "m2bronmask228";
const HAK_RESREF: &str = "m2bronh228v2";
const MODULE_RESREF: &str = "m2bronpal228";
const MODULE_FILENAME: &str = "m2bronpal228.mod";
const MODULE_DISPLAY_NAME: &str = "Meshy2Aurora Bronze Mask Palette 228 V2";
const AREA_DISPLAY_NAME: &str = "Bronze Mask Item Palette 228 V2";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_root = env::var("M2A_BRONZE_MASK_228_OUTPUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            root.join("proof-output/tlc-bronze-mask-helmet-five-material-228-v2-20260905")
        });
    ...
    if !conversion.report.conversion_eligible {
        return Err(format!(
            "Bronze Mask 228 source failed exact static Item Profile A admission: {}",
    ...
    if (fitted_bounds.1[2] - (DONOR_BOUNDS_MAX[2] - TOP_CLEARANCE_METERS)).abs() > 0.000_01 {
        return Err(
            "Bronze Mask 228 fitted top differs from the accepted donor-top-fit policy".into(),
        );
    }
    ...
    let recipe = ItemAppearanceRecipeV1 {
        ...
        identity: ItemIdentityV1 {
            ...
            tag: "M2A_BRONZE_MASK_228_V2".to_owned(),
            display_name: "Brązowa Maska 228".to_owned(),
        },
        gender: None,
    ...
    {
        return Err("helmet naming did not resolve to the exact variant-228 namespace".into());
    }
    ...
    {
        return Err("Bronze Mask 228 MDL did not preserve the explicit double-sided geometry, class and stream partitioning".into());
    }
    ...
    if model_eye_luma_lift_count != 262_144 {
        return Err(
            "Bronze Mask 228 model PLT eye-layer lift did not cover the exact atlas region".into(),
        );
    }
    ...
    if icon_eye_luma_lift_count != 46 {
        return Err(
            "Bronze Mask 228 icon PLT eye-layer lift did not cover the exact eye silhouette".into(),
        );
    }
    ...
    if transparent_icon_pixels == 0 || !icon_border_is_transparent(&icon_plt_image) {
        return Err("Bronze Mask 228 icon PLT has no exact transparent background border".into());
    }
    ...
    let module_fixture_resources = build_helmet_228_module_fixture(&source_module)?;
    let package = write_item_package_v1(
        &ItemPackageBuildRequestV1 {
            schema_version: ITEM_SCHEMA_VERSION,
            generator_identity: "materialize_tlc_bronze_mask_helmet_228_v2".to_owned(),
            recipe: recipe.clone(),
            namespace,
            uti: ItemUtiBuildRequestV1 {
                ...
                properties: ItemUtiPropertiesV1 {
                    ...
                    identified_description: "Wariant 228 V2 z pięcioma regionami PLT i zachowaną semantyką doubleSided.".to_owned(),
                    cost: 1,
                    comment: "TLC Bronze Mask palette-only candidate 228 V2.".to_owned(),
                    palette_id: 9,
    ...
    {
        return Err(
            "Bronze Mask 228 package is not the exact palette-only ordered-HAK fixture".into(),
        );
    }
    ...
    let offline_report = serde_json::to_vec_pretty(&json!({
        ...
        "lineageDecision": {
            "previousHelm227V1": "owner_visible_with_eye_surface_missing_from_the_viewer_side",
            "relationship": "owner_authorized_double_sided_semantic_correction_after_visual_failure",
            "ownerInstruction": "ostatnie_demo_jest_fail_jezeli_poprawiles_oczy_to_mozna_wygenerowac_nowy",
        },
...
fn build_helmet_228_module_fixture(
    source_module: &[u8],
        ...
        match descriptor.resource_type {
            IFO_RESOURCE_TYPE => {
                ...
                replace_loc_string(
                    &mut document.root,
                    "Mod_Description",
                    "Palette-only Bronze Mask 228 V2 module: one model-named PLT atlas with Metal 1, Cloth 1/2 and Leather 1/2 regions, corrected native PLT icon and 84930 explicit double-sided triangles.",
                )?;
...
fn validate_package_readback(
    ...
    if hak.find(MODEL_RESREF, 3).is_ok() || hak.find(ICON_RESREF, 3).is_ok() {
        return Err(
            "Bronze Mask 228 HAK contains forbidden fallback TGA for a ModelType-1 helmet".into(),
        );
    }
    ...
    {
        return Err(
            "Bronze Mask 228 UTI readback lost variant 228 or the default PLT color rows".into(),
        );
    }
    Ok(())
}
