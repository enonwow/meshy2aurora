...
use m2a_core::animated_donor_candidate::{
    M0_R34_AREA_RESREF, M0_R34_HAK_RESREF, M0_R34_MODEL_RESREF, M0_R34_MODULE_RESREF,
    M0_R34_TEXTURE_RESREF, build_m0_r34_animated_donor_candidate_v1,
    verify_m0_r34_animated_donor_candidate_v1,
};
use sha2::{Digest, Sha256};

const CANONICAL_OUTPUT: &str =
    r"C:\Projects\meshy2aurora\proof-output\m0-r34-rig-only-state-20260724";
const CONTRACT_FILE: &str = "m0-r34-rig-only-state-lineage-contract-v1.json";
...
fn run() -> Result<String, String> {
    ...
    fs::create_dir_all(&generated).map_err(|error| format!("M2A-R34-OUTPUT-CREATE: {error}"))?;
    let outputs = [
        (
            generated.join(format!("{M0_R34_MODEL_RESREF}.mdl")),
            artifact.model.as_slice(),
        ),
        (
            generated.join(format!("{M0_R34_TEXTURE_RESREF}.tga")),
        ...
        (
            generated.join(format!("{M0_R34_HAK_RESREF}.hak")),
            artifact.hak.as_slice(),
        ),
        (
            generated.join(format!("{M0_R34_MODULE_RESREF}.mod")),
    ...
    for (path, expected) in &outputs {
        ...
        if actual != *expected {
            return Err(format!("M2A-R34-OUTPUT-READBACK: {}", path.display()));
        }
    }

    serde_json::to_string_pretty(&serde_json::json!({
        "ok": true,
        "status": "M0_R34_RIG_ONLY_STATE_CANDIDATE_OFFLINE_MATERIALIZED",
        "outputDirectory": command.output,
        "module": {
            "resref": M0_R34_MODULE_RESREF,
            "areaResref": M0_R34_AREA_RESREF,
            "byteLength": artifact.module.len(),
            "sha256": sha256(&artifact.module),
        },
        "orderedHakList": [{
            "resref": M0_R34_HAK_RESREF,
    ...
    .map_err(|error| format!("M2A-R34-SUMMARY: {error}"))
}
...
fn parse(arguments: impl IntoIterator<Item = String>) -> Result<Command, String> {
    ...
    while let Some(argument) = values.next() {
        let target = match argument.as_str() {
            ...
            "--out" => &mut output,
            _ => return Err(format!("M2A-R34-ARGUMENT: {argument}\n{}", usage())),
        };
        if target.is_some() {
            return Err(format!("M2A-R34-ARGUMENT-DUPLICATE: {argument}"));
        }
        *target = values.next().filter(|value| !value.starts_with("--"));
        if target.is_none() {
            return Err(format!("M2A-R34-ARGUMENT-MISSING: {argument}"));
        }
    }
    Ok(Command {
...
fn read(path: &Path, label: &str) -> Result<Vec<u8>, String> {
    fs::read(path).map_err(|error| format!("M2A-R34-{label}-READ {}: {error}", path.display()))
}
...
fn write_new(path: &Path, bytes: &[u8]) -> Result<(), String> {
    let mut file = fs::OpenOptions::new()
        ...
        .create_new(true)
        .open(path)
        .map_err(|error| format!("M2A-R34-OUTPUT-CREATE-NEW {}: {error}", path.display()))?;
    file.write_all(bytes)
        .and_then(|_| file.sync_all())
        .map_err(|error| format!("M2A-R34-OUTPUT-WRITE {}: {error}", path.display()))
}
...
fn usage() -> String {
    "usage: materialize_m0_r34_animated_donor_candidate --source-glb <exact-M0.glb> --donor-glb <exact-H1.glb> --appearance-2da <exact-full-table> --out <exact-r34-dir>".to_owned()
}
