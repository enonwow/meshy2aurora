...
use m2a_core::{
    ...
    item_material_mapping::{NwnItemPaletteLayerV1, lift_item_plt_layer_luma_v1},
    item_package::{
    ...
    item_part::{
        MeshyItemPartCompileRequestV1, compile_meshy_static_item_part_v2,
...
const SOURCE_SHA256: &str = "0b5cf58cda834916152a1c743e6efd8af051962aa974bd440c4e9131182c8195";
const OWNER_SOURCE_SHA256: &str =
    "1cd62de066d35f096f555aa3db40bb7ac16ed790570100b9c6c27aae48b02efd";
const MAPPING_SHA256: &str = "92fbc02382989ff404ecdaa607b6077d01db7314fbf7ab9554e0fc2bed475aac";
const ICON_BEAUTY_SHA256: &str = "f2ec4afa606b32b6ac492927d7078acd7dcb4214cbbe2561e3fe52a74b709b80";
const ICON_MATERIAL_ID_SHA256: &str =
    "9d6d79f1838181284f5a4250b58fa8f5b56eec1bad26033939c7e903afab92f0";
const SOURCE_TRIANGLES: usize = 42_465;
const COMPILED_TRIANGLES: usize = 84_930;
...
const VARIANT: u8 = 229;
const MODEL_RESREF: &str = "helm_229";
const ICON_RESREF: &str = "ihelm_229";
const UTI_RESREF: &str = "m2bronmask229";
const HAK_RESREF: &str = "m2bronh229v2";
const MODULE_RESREF: &str = "m2bronpal229";
const MODULE_FILENAME: &str = "m2bronpal229.mod";
const MODULE_DISPLAY_NAME: &str = "Meshy2Aurora Bronze Mask Palette 229 V2";
const AREA_DISPLAY_NAME: &str = "Bronze Mask Item Palette 229 V2";
...
const EYE_LUMA_MULTIPLIER: u16 = 3;
const EYE_LUMA_OFFSET: u8 = 32;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let source_path =
        root.join("sample-3d/tlc-bronze-mask-tripo-20260905-v3/source-palette-atlas-v2.glb");
    let icon_source_path = root.join(
        "artifacts/diagnostics/tlc-bronze-mask-tripo-20260905-v3-offline-qa/icon-source-v2/icon-beauty.png",
    );
    let icon_material_id_path = root.join(
        "artifacts/diagnostics/tlc-bronze-mask-tripo-20260905-v3-offline-qa/icon-source-v2/icon-material-id.png",
    );
    ...
    let output_root = env::var("M2A_BRONZE_MASK_229_OUTPUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            root.join("proof-output/tlc-bronze-mask-helmet-five-material-229-v2-20260905")
        });
    ...
    if !conversion.report.conversion_eligible {
        return Err(format!(
            "Bronze Mask 229 source failed exact static Item Profile A admission: {}",
    ...
    if (fitted_bounds.1[2] - (DONOR_BOUNDS_MAX[2] - TOP_CLEARANCE_METERS)).abs() > 0.000_01 {
        return Err(
            "Bronze Mask 229 fitted top differs from the accepted donor-top-fit policy".into(),
        );
    }
    ...
    let recipe = ItemAppearanceRecipeV1 {
        ...
        identity: ItemIdentityV1 {
            ...
            tag: "M2A_BRONZE_MASK_229_V2".to_owned(),
            display_name: "Brązowa Maska 229".to_owned(),
        },
        gender: None,
        parts: vec![ItemPartRecipeV1 {
            slot: ItemPartSlotV1::Model,
            variant: VARIANT,
            source_part_id: "tlc-bronze-mask-tripo-20260905-v3/source-palette-atlas-v2.glb"
                .to_owned(),
    ...
    {
        return Err("helmet naming did not resolve to the exact variant-229 namespace".into());
    }
    let recipe_sha256 = item_recipe_sha256_v1(&recipe)?;

    eprintln!("compiling {MODEL_RESREF} from {SOURCE_TRIANGLES} source triangles");
    let compiled = compile_meshy_static_item_part_v2(
        &source_glb,
    ...
    if compiled.report.triangle_count != COMPILED_TRIANGLES
        || compiled.report.source_triangle_count != SOURCE_TRIANGLES
        || compiled.report.double_sided_backface_triangle_count != SOURCE_TRIANGLES
        || compiled.report.double_sided_policy != "SOURCE_DOUBLE_SIDED_EXPLICIT_BACKFACES_V1"
        || compiled.inspection.model.classification != 4
        || compiled.report.segmentation.output_segment_count <= 1
    {
        return Err("Bronze Mask 229 MDL did not preserve the explicit double-sided geometry, class and stream partitioning".into());
    }
    ...
    let mut model_plt_image = build_model_plt(&diffuse_image)?;
    let model_eye_luma_lift_count = lift_item_plt_layer_luma_v1(
        &mut model_plt_image,
        NwnItemPaletteLayerV1::Cloth2,
        EYE_LUMA_MULTIPLIER,
        EYE_LUMA_OFFSET,
    )?;
    if model_eye_luma_lift_count != 262_144 {
        return Err(
            "Bronze Mask 229 model PLT eye-layer lift did not cover the exact atlas region".into(),
        );
    }
    ...
    let mut icon_plt_image = build_icon_plt(&icon_source_path, &icon_material_id_path)?;
    let icon_eye_luma_lift_count = lift_item_plt_layer_luma_v1(
        &mut icon_plt_image,
        NwnItemPaletteLayerV1::Cloth2,
        EYE_LUMA_MULTIPLIER,
        EYE_LUMA_OFFSET,
    )?;
    if icon_eye_luma_lift_count != 33 {
        return Err(format!(
            "Bronze Mask 229 icon PLT eye-layer lift covered {icon_eye_luma_lift_count} pixels instead of the exact expected eye silhouette"
        )
        .into());
    }
    ...
    if transparent_icon_pixels == 0 || !icon_border_is_transparent(&icon_plt_image) {
        return Err("Bronze Mask 229 icon PLT has no exact transparent background border".into());
    }
    ...
    let module_fixture_resources = build_helmet_229_module_fixture(&source_module)?;
    let package = write_item_package_v1(
        &ItemPackageBuildRequestV1 {
            schema_version: ITEM_SCHEMA_VERSION,
            generator_identity: "materialize_tlc_bronze_mask_helmet_229_v2".to_owned(),
            recipe: recipe.clone(),
            namespace,
            uti: ItemUtiBuildRequestV1 {
                ...
                properties: ItemUtiPropertiesV1 {
                    ...
                    identified_description: "Wariant 229 V2 z pięcioma regionami PLT i zachowaną semantyką doubleSided.".to_owned(),
                    cost: 1,
                    comment: "TLC Bronze Mask palette-only candidate 229 V2.".to_owned(),
                    palette_id: 9,
    ...
    {
        return Err(
            "Bronze Mask 229 package is not the exact palette-only ordered-HAK fixture".into(),
        );
    }
    ...
    let offline_report = serde_json::to_vec_pretty(&json!({
        ...
        "lineageDecision": {
            "previousHelm228V2": "owner_reported_visually_successful",
            "relationship": "owner_approved_successor_using_updated_five_material_source",
            "ownerInstruction": "nowa_wersja_modelu_i_dema_approved_z_jawnym_mapowaniem_pieciu_partow",
        },
        ...
        "paletteAtlas": {
            ...
            "materialMappings": [
                ...
                {"sourceMaterialId": 1, "semantic": "mask-trim", "layer": "Leather 2", "layerIndex": 7, "pixelRectTopLeft": [1024, 0, 1024, 1024]},
                {"sourceMaterialId": 2, "semantic": "main-mask-shell", "layer": "Metal 1", "layerIndex": 2, "pixelRectTopLeft": [0, 1024, 512, 512]},
                {"sourceMaterialId": 3, "semantic": "forehead-wrap", "layer": "Leather 1", "layerIndex": 6, "pixelRectTopLeft": [512, 1024, 512, 512]},
                {"sourceMaterialId": 4, "semantic": "eye-inserts", "layer": "Cloth 2", "layerIndex": 5, "pixelRectTopLeft": [1024, 1024, 512, 512]},
            ],
            "policy": "five_source_material_regions_remapped_to_one_model_named_plt",
        },
        "model": {
            "sha256": sha256_hex(&compiled.payload),
            "sourceTriangleCount": compiled.report.source_triangle_count,
            "triangleCount": compiled.report.triangle_count,
            "doubleSidedBackfaceTriangleCount": compiled.report.double_sided_backface_triangle_count,
            "doubleSidedPolicy": compiled.report.double_sided_policy,
        ...
        "eyeCloth2LumaLift": {
            "targetLayer": "Cloth 2",
            "targetLayerIndex": 5,
            "formula": "min(174, original * 3 + 32)",
            "modelPixelsChanged": model_eye_luma_lift_count,
            "iconPixelsChanged": icon_eye_luma_lift_count,
            "geometryAndUvChanged": false,
            "otherPaletteLayersChanged": false,
        },
        "modelPlt": {
...
fn build_helmet_229_module_fixture(
    source_module: &[u8],
        ...
        match descriptor.resource_type {
            IFO_RESOURCE_TYPE => {
                ...
                replace_loc_string(
                    &mut document.root,
                    "Mod_Description",
                    "Palette-only Bronze Mask 229 V2 module: one model-named PLT atlas with Metal 1, Cloth 1/2 and Leather 1/2 regions, corrected native PLT icon and 84930 explicit double-sided triangles.",
                )?;
...
fn validate_package_readback(
    ...
    if hak.find(MODEL_RESREF, 3).is_ok() || hak.find(ICON_RESREF, 3).is_ok() {
        return Err(
            "Bronze Mask 229 HAK contains forbidden fallback TGA for a ModelType-1 helmet".into(),
        );
    }
    ...
    {
        return Err(
            "Bronze Mask 229 UTI readback lost variant 229 or the default PLT color rows".into(),
        );
    }
    Ok(())
}
