...
const VARIANT: u8 = 227;
const MODEL_RESREF: &str = "helm_227";
const ICON_RESREF: &str = "ihelm_227";
const UTI_RESREF: &str = "m2bronmask227";
const HAK_RESREF: &str = "m2bronh227v1";
const MODULE_RESREF: &str = "m2bronpal227";
const MODULE_FILENAME: &str = "m2bronpal227.mod";
const MODULE_DISPLAY_NAME: &str = "Meshy2Aurora Bronze Mask Palette 227 V1";
const AREA_DISPLAY_NAME: &str = "Bronze Mask Item Palette 227 V1";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_root = env::var("M2A_BRONZE_MASK_227_OUTPUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            root.join("proof-output/tlc-bronze-mask-helmet-five-material-227-v1-20260905")
        });
    ...
    if !conversion.report.conversion_eligible {
        return Err(format!(
            "Bronze Mask 227 source failed exact static Item Profile A admission: {}",
    ...
    if (fitted_bounds.1[2] - (DONOR_BOUNDS_MAX[2] - TOP_CLEARANCE_METERS)).abs() > 0.000_01 {
        return Err(
            "Bronze Mask 227 fitted top differs from the accepted donor-top-fit policy".into(),
        );
    }
    ...
    let recipe = ItemAppearanceRecipeV1 {
        ...
        identity: ItemIdentityV1 {
            ...
            tag: "M2A_BRONZE_MASK_227_V1".to_owned(),
            display_name: "Brązowa Maska 227".to_owned(),
        },
        gender: None,
    ...
    {
        return Err("helmet naming did not resolve to the exact variant-227 namespace".into());
    }
    ...
    {
        return Err("Bronze Mask 227 MDL did not preserve the sanitized geometry, class and stream partitioning".into());
    }
    ...
    if model_eye_luma_lift_count != 262_144 {
        return Err(
            "Bronze Mask 227 model PLT eye-layer lift did not cover the exact atlas region".into(),
        );
    }
    ...
    if icon_eye_luma_lift_count != 46 {
        return Err(
            "Bronze Mask 227 icon PLT eye-layer lift did not cover the exact eye silhouette".into(),
        );
    }
    ...
    if transparent_icon_pixels == 0 || !icon_border_is_transparent(&icon_plt_image) {
        return Err("Bronze Mask 227 icon PLT has no exact transparent background border".into());
    }
    ...
    let module_fixture_resources = build_helmet_227_module_fixture(&source_module)?;
    let package = write_item_package_v1(
        &ItemPackageBuildRequestV1 {
            schema_version: ITEM_SCHEMA_VERSION,
            generator_identity: "materialize_tlc_bronze_mask_helmet_227_v1".to_owned(),
            recipe: recipe.clone(),
            namespace,
            uti: ItemUtiBuildRequestV1 {
                ...
                properties: ItemUtiPropertiesV1 {
                    ...
                    identified_description: "Wariant 227 V1 z pięcioma jawnie przypisanymi regionami jednego atlasu PLT.".to_owned(),
                    cost: 1,
                    comment: "TLC Bronze Mask palette-only candidate 227 V1.".to_owned(),
                    palette_id: 9,
    ...
    {
        return Err(
            "Bronze Mask 227 package is not the exact palette-only ordered-HAK fixture".into(),
        );
    }
    ...
    let offline_report = serde_json::to_vec_pretty(&json!({
        ...
        "lineageDecision": {
            "previousHelm226V1": "owner_visible_with_cloth2_eye_luma_failure",
            "relationship": "owner_authorized_exact_eye_luma_correction",
            "ownerOverride": "generate_the_corrected_version_after_iteration_gate_was_explained",
        },
...
fn build_helmet_227_module_fixture(
    source_module: &[u8],
        ...
        match descriptor.resource_type {
            IFO_RESOURCE_TYPE => {
                ...
                replace_loc_string(
                    &mut document.root,
                    "Mod_Description",
                    "Palette-only Bronze Mask 227 V1 module: one model-named PLT atlas with Metal 1, Cloth 1/2 and Leather 1/2 regions, corrected native PLT icon and 42465 triangles.",
                )?;
...
fn validate_package_readback(
    ...
    if hak.find(MODEL_RESREF, 3).is_ok() || hak.find(ICON_RESREF, 3).is_ok() {
        return Err(
            "Bronze Mask 227 HAK contains forbidden fallback TGA for a ModelType-1 helmet".into(),
        );
    }
    ...
    {
        return Err(
            "Bronze Mask 227 UTI readback lost variant 227 or the default PLT color rows".into(),
        );
    }
    Ok(())
}
