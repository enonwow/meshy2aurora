...
use m2a_core::animated_donor_candidate::{
    H2_R43_AREA_RESREF, H2_R43_HAK_RESREF, H2_R43_MODEL_RESREF, H2_R43_MODULE_RESREF,
    H2_R43_TEXTURE_RESREF, build_h2_r43_h1_root_layout_candidate_v1,
    verify_h2_r43_h1_root_layout_candidate_v1,
};
use sha2::{Digest, Sha256};

const CANONICAL_OUTPUT: &str =
    r"C:\Projects\meshy2aurora\proof-output\h2-r43-h1-v20-root-rigid-type0-20260724";
const CONTRACT_FILE: &str = "h2-r43-h1-v20-root-rigid-type0-lineage-contract-v1.json";
...
fn run() -> Result<String, String> {
    ...
    if command.output.as_path() != Path::new(CANONICAL_OUTPUT) {
        return Err(format!(
            "M2A-H2-R43-OUTPUT-IDENTITY: exact output must be {CANONICAL_OUTPUT}"
        ));
    }
    if command.output.exists() {
        return Err(format!(
            "M2A-H2-R43-OUTPUT-EXISTS: {}",
            command.output.display()
        ));
    }
    let source = read(&command.source_glb, "source")?;
    let appearance = read(&command.appearance_two_da, "appearance")?;
    let artifact = build_h2_r43_h1_root_layout_candidate_v1(&source, &appearance)
    ...
    fs::create_dir_all(&generated).map_err(|error| format!("M2A-H2-R43-OUTPUT-CREATE: {error}"))?;
    let outputs = [
        (
            generated.join(format!("{H2_R43_MODEL_RESREF}.mdl")),
            artifact.model.as_slice(),
        ),
        (
            generated.join(format!("{H2_R43_TEXTURE_RESREF}.tga")),
        ...
        (
            generated.join(format!("{H2_R43_HAK_RESREF}.hak")),
            artifact.hak.as_slice(),
        ),
        (
            generated.join(format!("{H2_R43_MODULE_RESREF}.mod")),
    ...
    for (path, expected) in &outputs {
        ...
        if actual != *expected {
            return Err(format!("M2A-H2-R43-OUTPUT-READBACK: {}", path.display()));
        }
    }

    serde_json::to_string_pretty(&serde_json::json!({
        "ok": true,
        "status": "H2_R43_H1_V20_ROOT_RIGID_TYPE0_CANDIDATE_OFFLINE_MATERIALIZED",
        "outputDirectory": command.output,
        "module": {
            "fileName": format!("{H2_R43_MODULE_RESREF}.mod"),
            "resref": H2_R43_MODULE_RESREF,
            "areaResref": H2_R43_AREA_RESREF,
            "byteLength": artifact.module.len(),
            "sha256": sha256(&artifact.module),
        },
        "orderedHakList": [{
            "fileName": format!("{H2_R43_HAK_RESREF}.hak"),
            "resref": H2_R43_HAK_RESREF,
        ...
        "baseRootControllerCount": artifact.contract.base_root_controller_count,
        "formatProfile": artifact.contract.format_profile,
    ...
    .map_err(|error| format!("M2A-H2-R43-SUMMARY: {error}"))
}
...
fn parse(arguments: impl IntoIterator<Item = String>) -> Result<Command, String> {
    ...
    while let Some(argument) = values.next() {
        let target = match argument.as_str() {
            ...
            "--out" => &mut output,
            _ => {
                return Err(format!("M2A-H2-R43-ARGUMENT: {argument}\n{}", usage()));
            }
        };
        if target.is_some() {
            return Err(format!("M2A-H2-R43-ARGUMENT-DUPLICATE: {argument}"));
        }
        *target = values.next().filter(|value| !value.starts_with("--"));
        if target.is_none() {
            return Err(format!("M2A-H2-R43-ARGUMENT-MISSING: {argument}"));
        }
    }
    Ok(Command {
...
fn read(path: &Path, label: &str) -> Result<Vec<u8>, String> {
    fs::read(path).map_err(|error| format!("M2A-H2-R43-{label}-READ {}: {error}", path.display()))
}
...
fn write_new(path: &Path, bytes: &[u8]) -> Result<(), String> {
    let mut file = fs::OpenOptions::new()
        ...
        .create_new(true)
        .open(path)
        .map_err(|error| format!("M2A-H2-R43-OUTPUT-CREATE-NEW {}: {error}", path.display()))?;
    file.write_all(bytes)
        .and_then(|_| file.sync_all())
        .map_err(|error| format!("M2A-H2-R43-OUTPUT-WRITE {}: {error}", path.display()))
}
...
fn usage() -> String {
    "usage: materialize_h2_r43_h1_root_layout_candidate --source-glb <exact-H2.glb> --appearance-2da <exact-full-table> --out <exact-r43-dir>".to_owned()
}
