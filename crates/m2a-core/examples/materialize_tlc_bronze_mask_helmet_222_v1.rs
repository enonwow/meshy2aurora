...
const VARIANT: u8 = 222;
const MODEL_RESREF: &str = "helm_222";
const ICON_RESREF: &str = "ihelm_222";
const UTI_RESREF: &str = "m2bronmask222";
const HAK_RESREF: &str = "m2bronh222v1";
const MODULE_RESREF: &str = "m2bronpal222";
const MODULE_FILENAME: &str = "m2bronpal222.mod";
const MODULE_DISPLAY_NAME: &str = "Meshy2Aurora Bronze Mask Palette 222";
const AREA_DISPLAY_NAME: &str = "Bronze Mask Item Palette 222";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let output_root = env::var("M2A_BRONZE_MASK_222_OUTPUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            root.join("proof-output/tlc-bronze-mask-helmet-palette-only-222-v1-20260904")
        });
    ...
    if !conversion.report.conversion_eligible {
        return Err(format!(
            "Bronze Mask 222 source failed exact static Item Profile A admission: {}",
    ...
    let recipe = ItemAppearanceRecipeV1 {
        ...
        identity: ItemIdentityV1 {
            ...
            tag: "M2A_BRONZE_MASK_222_V1".to_owned(),
            display_name: "Brązowa Maska 222".to_owned(),
        },
        gender: None,
    ...
    {
        return Err("helmet naming did not resolve to the exact variant-222 namespace".into());
    }
    ...
    {
        return Err("Bronze Mask 222 MDL did not preserve the sanitized geometry, class and stream partitioning".into());
    }
    ...
    if transparent_icon_pixels == 0 || !icon_border_is_transparent(&icon_plt_image) {
        return Err("Bronze Mask 222 icon PLT has no exact transparent background border".into());
    }
    ...
    let module_fixture_resources = build_helmet_222_module_fixture(&source_module)?;
    let package = write_item_package_v1(
        &ItemPackageBuildRequestV1 {
            schema_version: ITEM_SCHEMA_VERSION,
            generator_identity: "materialize_tlc_bronze_mask_helmet_222_v1".to_owned(),
            recipe: recipe.clone(),
            namespace,
            uti: ItemUtiBuildRequestV1 {
                ...
                properties: ItemUtiPropertiesV1 {
                    ...
                    identified_description:
                        "Wariant 222: model Tripo, natywne PLT, ikona PLT i skala helm_035."
                            .to_owned(),
                    cost: 1,
                    comment: "TLC Bronze Mask palette-only candidate 222 V1.".to_owned(),
                    palette_id: 9,
    ...
    {
        return Err(
            "Bronze Mask 222 package is not the exact palette-only ordered-HAK fixture".into(),
        );
    }
    ...
    let offline_report = serde_json::to_vec_pretty(&json!({
        ...
        "lineageDecision": {
            "previousHelm221R3": "frozen_functionally_satisfactory_but_visual_model_rejected_by_owner",
            "relationship": "distinct_owner_selected_source_and_resource_lineage",
        },
...
fn build_helmet_222_module_fixture(
    source_module: &[u8],
        ...
        match descriptor.resource_type {
            IFO_RESOURCE_TYPE => {
                ...
                replace_loc_string(
                    &mut document.root,
                    "Mod_Description",
                    "Palette-only Bronze Mask 222 module: Tripo source, PLT model/icon, Metal 1 plus Cloth 1, donor-calibrated scale and 36365 sanitized triangles.",
                )?;
...
fn validate_package_readback(
    ...
    if hak.find(MODEL_RESREF, 3).is_ok() || hak.find(ICON_RESREF, 3).is_ok() {
        return Err(
            "Bronze Mask 222 HAK contains forbidden fallback TGA for a ModelType-1 helmet".into(),
        );
    }
    ...
    {
        return Err(
            "Bronze Mask 222 UTI readback lost variant 222 or the default PLT color rows".into(),
        );
    }
    Ok(())
}
