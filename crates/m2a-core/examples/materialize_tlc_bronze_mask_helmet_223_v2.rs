...
use image::{GrayImage, ImageReader, RgbaImage, imageops::FilterType};
use m2a_core::{
    erf::ErfArchive,
    gff::{
...
const SOURCE_SHA256: &str = "1f032f9388fc7c94522669938a35855d4c2037d7321fd112730ed644e9c419d3";
...
const SEMANTIC_MASK_SHA256: &str =
    "ff5716185cbd534654289d8278afab94916b4d80a88fdeaa3985e2cc73fa1cb7";

const VARIANT: u8 = 223;
const MODEL_RESREF: &str = "helm_223";
const ICON_RESREF: &str = "ihelm_223";
const UTI_RESREF: &str = "m2bronmask223";
const HAK_RESREF: &str = "m2bronh223v2";
const MODULE_RESREF: &str = "m2bronpal223";
const MODULE_FILENAME: &str = "m2bronpal223.mod";
const MODULE_DISPLAY_NAME: &str = "Meshy2Aurora Bronze Mask Palette 223 V2";
const AREA_DISPLAY_NAME: &str = "Bronze Mask Item Palette 223 V2";
...
const GROUND_CLEARANCE_METERS: f32 = 0.002;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let source_path = root.join("sample-3d/tlc-bronze-mask-tripo-20260904-v1/source-clean-v1.glb");
    let icon_source_path = root.join(
        "artifacts/diagnostics/tlc-bronze-mask-tripo-20260904-v1-offline-qa/renders/front.png",
    );
    let semantic_mask_path = root.join(
        "artifacts/diagnostics/tlc-bronze-mask-tripo-20260904-v2-layer-mask/semantic-metal1-mask.png",
    );
    ...
    let output_root = env::var("M2A_BRONZE_MASK_223_OUTPUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            root.join("proof-output/tlc-bronze-mask-helmet-palette-only-223-v2-20260904")
        });
    ...
    if !conversion.report.conversion_eligible {
        return Err(format!(
            "Bronze Mask 223 source failed exact static Item Profile A admission: {}",
    ...
    let transform = donor_width_grounded_transform(source_bounds)?;
    let fitted_bounds = transformed_bounds(source_bounds, transform);
    if (fitted_bounds.0[2] - GROUND_CLEARANCE_METERS).abs() > 0.000_01 {
        return Err("Bronze Mask 223 fitted minimum Z is not ground-aligned".into());
    }
    ...
    let recipe = ItemAppearanceRecipeV1 {
        ...
        identity: ItemIdentityV1 {
            ...
            tag: "M2A_BRONZE_MASK_223_V2".to_owned(),
            display_name: "Brązowa Maska 223".to_owned(),
        },
        gender: None,
        parts: vec![ItemPartRecipeV1 {
            slot: ItemPartSlotV1::Model,
            variant: VARIANT,
            source_part_id: "tlc-bronze-mask-tripo-20260904-v1/source-clean-v1.glb".to_owned(),
    ...
    {
        return Err("helmet naming did not resolve to the exact variant-223 namespace".into());
    }
    ...
    {
        return Err("Bronze Mask 223 MDL did not preserve the sanitized geometry, class and stream partitioning".into());
    }
    ...
    let semantic_mask_bytes = read_exact_hash(&semantic_mask_path, SEMANTIC_MASK_SHA256)?;
    let semantic_mask = image::load_from_memory(&semantic_mask_bytes)?.into_luma8();
    let model_plt_image = build_model_plt(&diffuse_image, &semantic_mask)?;
    ...
    let icon_plt_image = build_icon_plt(&icon_source_path)?;
    ...
    if transparent_icon_pixels == 0 || !icon_border_is_transparent(&icon_plt_image) {
        return Err("Bronze Mask 223 icon PLT has no exact transparent background border".into());
    }
    ...
    let module_fixture_resources = build_helmet_223_module_fixture(&source_module)?;
    let package = write_item_package_v1(
        &ItemPackageBuildRequestV1 {
            schema_version: ITEM_SCHEMA_VERSION,
            generator_identity: "materialize_tlc_bronze_mask_helmet_223_v2".to_owned(),
            recipe: recipe.clone(),
            namespace,
            uti: ItemUtiBuildRequestV1 {
                ...
                properties: ItemUtiPropertiesV1 {
                    description: "Kolorowalna brązowa maska z warstwami Metal 1 i Cloth 1."
                        .to_owned(),
                    identified_description: "Wariant 223 V2: spójne regiony Metal 1/Cloth 1, natywna ikona PLT i korekta położenia na gruncie.".to_owned(),
                    cost: 1,
                    comment: "TLC Bronze Mask palette-only candidate 223 V2.".to_owned(),
                    palette_id: 9,
    ...
    {
        return Err(
            "Bronze Mask 223 package is not the exact palette-only ordered-HAK fixture".into(),
        );
    }
    ...
    let offline_report = serde_json::to_vec_pretty(&json!({
        ...
        "lineageDecision": {
            "previousHelm222V1": "owner_visible_but_failed_cloth_metal_bleed_and_ground_sink",
            "relationship": "fresh_owner_failure_admitted_minimal_corrective_resref",
        },
        ...
        "source": {
            ...
            "sourceOriginalSha256": "17ea0a4c777352885d81d311c7c6996bad2d3106e62c2f11d990bc3eecc429b5",
        ...
        "fit": {
            ...
            "groundClearanceMeters": GROUND_CLEARANCE_METERS,
            "verticalPlacementPolicy": "minimum_model_z_equals_ground_clearance",
            "independentAxisScalingApplied": false,
        },
        "semanticLayerMask": {
            "path": semantic_mask_path.to_string_lossy(),
            "sha256": SEMANTIC_MASK_SHA256,
            "policy": "face_shell_region_in_source_geometry_space",
            "sourceMinimumHeight": 0.08,
            "sourceMaximumHeight": 0.60,
            "sourceMinimumFrontDepth": 0.10,
        },
        "model": {
...
fn donor_width_grounded_transform(
    source: ([f32; 3], [f32; 3]),
    ...
    Ok(ItemPartTransformV1 {
        translation: [
            ...
            GROUND_CLEARANCE_METERS - source.0[2] * scale,
        ],
...
fn build_model_plt(
    ...
    if semantic_mask.width() != source.width || semantic_mask.height() != source.height {
        return Err("semantic layer mask dimensions differ from the source texture".into());
    }
    let pixels = source
        .pixels
        .chunks_exact(channels)
        .zip(semantic_mask.pixels())
        .map(|(rgba, mask)| {
            let mut pixel = material_pixel(rgba[0], rgba[1], rgba[2]);
            pixel.layer_index = if mask[0] >= 128 { 2 } else { 4 };
            pixel
        })
        .collect();
    Ok(PltImageV1 {
...
fn build_icon_plt(path: &Path) -> Result<PltImageV1, Box<dyn std::error::Error>> {
    ...
    for rgba in resized.pixels() {
        if rgba[3] < 64 {
            pixels.push(PLT_TRANSPARENT_PIXEL_V1);
        } else {
            pixels.push(material_pixel(rgba[0], rgba[1], rgba[2]));
        }
    }
...
fn material_pixel(red: u8, green: u8, blue: u8) -> PltPixelV1 {
    ...
    PltPixelV1 {
        color_index,
        layer_index: if warm_metal { 2 } else { 4 },
    }
}
...
fn build_helmet_223_module_fixture(
    source_module: &[u8],
        ...
        match descriptor.resource_type {
            IFO_RESOURCE_TYPE => {
                ...
                replace_loc_string(
                    &mut document.root,
                    "Mod_Description",
                    "Palette-only Bronze Mask 223 V2 module: coherent face-shell Metal 1 mask, Cloth 1 cowl, ground-aligned minimum Z, native PLT icon and 36365 sanitized triangles.",
                )?;
...
fn validate_package_readback(
    ...
    if hak.find(MODEL_RESREF, 3).is_ok() || hak.find(ICON_RESREF, 3).is_ok() {
        return Err(
            "Bronze Mask 223 HAK contains forbidden fallback TGA for a ModelType-1 helmet".into(),
        );
    }
    ...
    {
        return Err(
            "Bronze Mask 223 UTI readback lost variant 223 or the default PLT color rows".into(),
        );
    }
    Ok(())
}
