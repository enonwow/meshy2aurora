...
const SOURCE_SHA256: &str = "b0e791d1606d81dccebd4f425bf60c18f2e8e0f51d22ff64fe8e75629bc437a1";
const SOURCE_TRIANGLES: usize = 100_000;
...
const HAK_RESREF: &str = "m2assmh221r3";
const MODULE_RESREF: &str = "m2assmpal221r3";
const MODULE_FILENAME: &str = "m2assmpal221r3.mod";
const MODULE_DISPLAY_NAME: &str = "Meshy2Aurora Sandstorm Mask Palette 221 R3";
const AREA_DISPLAY_NAME: &str = "Sandstorm Mask Item Palette 221 R3";
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let source_path =
        root.join("sample-3d/tlc-sandstorm-mask-meshy-0904135727-v1/source-p100k-r3.glb");
    ...
    let output_root = env::var("M2A_ITEM_PALETTE_R3_OUTPUT")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            root.join("proof-output/tlc-sandstorm-mask-helmet-palette-only-221-r3-20260904")
        });
    ...
    if ingest.report.statistics.triangle_count != SOURCE_TRIANGLES {
        return Err(format!(
            "expected exact {SOURCE_TRIANGLES}-triangle R3 source, got {}",
    ...
    if !conversion.report.conversion_eligible {
        return Err("R3 source failed exact static Item Profile A admission".into());
    }
    ...
    let recipe = ItemAppearanceRecipeV1 {
        ...
        identity: ItemIdentityV1 {
            ...
            tag: "M2A_SSMASK_PAL_221_R3".to_owned(),
            display_name: "Brązowa Maska Burzy Piaskowej R3".to_owned(),
        },
        gender: None,
        parts: vec![ItemPartRecipeV1 {
            slot: ItemPartSlotV1::Model,
            variant: VARIANT,
            source_part_id: "tlc-sandstorm-mask-meshy-0904135727-v1/source-p100k-r3.glb".to_owned(),
    ...
    {
        return Err("helmet naming did not resolve to the exact variant-221 namespace".into());
    }
    ...
    if compiled.report.triangle_count != SOURCE_TRIANGLES
        || compiled.inspection.model.classification != 4
        || compiled.report.segmentation.output_segment_count <= 1
    {
        return Err("R3 MDL did not preserve exact geometry, class and stream partitioning".into());
    }
    ...
    if transparent_icon_pixels == 0 || !icon_border_is_transparent(&icon_plt_image) {
        return Err("R3 icon PLT has no exact transparent background border".into());
    }
    ...
    let module_fixture_resources = build_r3_module_fixture(&source_module)?;
    let package = write_item_package_v1(
        &ItemPackageBuildRequestV1 {
            schema_version: ITEM_SCHEMA_VERSION,
            generator_identity: "materialize_tlc_sandstorm_mask_palette_only_r3".to_owned(),
            recipe: recipe.clone(),
            namespace,
            uti: ItemUtiBuildRequestV1 {
                ...
                properties: ItemUtiPropertiesV1 {
                    description: "Kolorowalna maska z warstwami Metal 1 i Cloth 1.".to_owned(),
                    identified_description:
                        "R3: natywne PLT, ikona PLT, retailowa skala i lżejsza geometria."
                            .to_owned(),
                    cost: 1,
                    comment: "TLC Sandstorm Mask palette-only candidate 221 R3.".to_owned(),
                    palette_id: 9,
    ...
    {
        return Err("R3 package is not the exact palette-only ordered-HAK fixture".into());
    }
    ...
    let offline_report = serde_json::to_vec_pretty(&json!({
        ...
        "sourceFailureR2": {
            ...
            "qualityFailures": ["ground_view_silver", "colors_ignored", "inventory_icon_black", "scale_too_small", "toolset_manipulation_unresponsive"],
        },
...
fn build_model_plt(source: &TgaImageV1) -> Result<PltImageV1, Box<dyn std::error::Error>> {
    ...
    let pixels = source
        ...
        .map(|rgba| material_pixel(rgba[0], rgba[1], rgba[2]))
        .collect();
    Ok(PltImageV1 {
        schema_version: 1,
        width: source.width,
...
fn build_r3_module_fixture(
    ...
    for descriptor in archive.resources() {
        ...
        if descriptor.resource_type == UTC_RESOURCE_TYPE {
            return Err("R2 palette-only source unexpectedly contains a creature blueprint".into());
        }
        ...
        match descriptor.resource_type {
            IFO_RESOURCE_TYPE => {
                ...
                replace_loc_string(
                    &mut document.root,
                    "Mod_Description",
                    "Palette-only R3 helmet module: PLT model/icon, Metal 1 plus Cloth 1, donor-calibrated scale and 100000-triangle interaction profile.",
                )?;
...
fn validate_package_readback(
    ...
    if hak.find(MODEL_RESREF, 3).is_ok() || hak.find(ICON_RESREF, 3).is_ok() {
        return Err("R3 HAK contains forbidden fallback TGA for a ModelType-1 helmet".into());
    }
    ...
    {
        return Err("R3 UTI readback lost variant 221 or the default PLT color rows".into());
    }
    Ok(())
}
