...
use m2a_core::{
    animated_donor_candidate::{
        M0_R39_ACCEPTED_R38_PROOF_SHA256, M0_R39_APPEARANCE_ROW, M0_R39_AREA_RESREF,
        M0_R39_HAK_RESREF, M0_R39_MODEL_RESREF, M0_R39_MODEL_SHA256, M0_R39_MODULE_RESREF,
...
fn exact_m0_r39_controllerless_identity_root_candidate_is_deterministic_and_runtime_complete() {
    ...
    assert_eq!(
        artifact.contract.admitted_by_r38_proof_sha256,
        M0_R39_ACCEPTED_R38_PROOF_SHA256
    );
    assert_eq!(artifact.contract.model.resref, M0_R39_MODEL_RESREF);
    assert_eq!(artifact.contract.model.sha256, M0_R39_MODEL_SHA256);
    ...
    assert_eq!(
        artifact.contract.skin_readback.base_root_controller_count,
        0
    );
    assert_eq!(
    ...
    let hak = ErfArchive::parse(&artifact.hak).expect("r39 HAK");
    assert_eq!(hak.resources().len(), 3);
    assert_eq!(hak.find(M0_R39_MODEL_RESREF, 2002).unwrap(), artifact.model);
    assert_eq!(
        hak.find(M0_R39_TEXTURE_RESREF, 3).unwrap(),
        artifact.texture
    );
    assert_eq!(
    ...
    let scene = inspect_binary_creature_multi_fixture_module_v1(&artifact.module).expect("r39 MOD");
    assert_eq!(scene.module_resref, M0_R39_MODULE_RESREF);
    assert_eq!(scene.area_resref, M0_R39_AREA_RESREF);
    assert_eq!(scene.ordered_hak_resrefs, [M0_R39_HAK_RESREF]);
    assert_eq!(scene.fixtures.len(), 1);
    assert_eq!(scene.fixtures[0].appearance_row, M0_R39_APPEARANCE_ROW);

    let mut wrong_source = source.clone();
    *wrong_source.last_mut().expect("source byte") ^= 1;
    let error = build_m0_r39_animated_donor_candidate_v1(&wrong_source, &donor, &appearance)
        .expect_err("source identity attack");
    assert_eq!(error.code, "M2A-R39-INPUT-IDENTITY");

    let mut wrong_contract = artifact.contract.clone();
    wrong_contract.model.sha256 = "0".repeat(64);
    let error = verify_m0_r39_animated_donor_candidate_v1(
        &wrong_contract,
        &source,
        &donor,
        &appearance,
    ...
    assert_eq!(error.code, "M2A-R39-REPLAY-MISMATCH");
}
