...
#[test]
fn legacy_custom_weapon_clone_remains_available_outside_reserved_baseitem_113() {
    let mut source = baseitems_ready_for_exact_113_append();
    source.extend_from_slice(
        b"113 reserved_113 **** Ring 0 0 it_bag iring 8 1 1 **** **** **** **** ****\n",
    );
    let mut request = hextech_baseitem_request(114);
    request.label = "legacy_arc_cannon".to_owned();
    request.item_class = "WArc".to_owned();
    let artifact = append_item_custom_weapon_baseitem_v2(&source, &request).unwrap();

    assert_eq!(artifact.report.status, "APPENDED_EXACT");
    assert_eq!(artifact.report.donor_base_item, 6);
    assert_eq!(artifact.report.output_base_item, 114);
    assert_eq!(artifact.report.runtime_route.base_item, 6);
    assert_eq!(artifact.report.runtime_route.runtime_clip, "xbowshot");
    assert_eq!(artifact.selected.base_item, 114);
    assert_eq!(artifact.selected.label, "legacy_arc_cannon");
    assert_eq!(artifact.selected.item_class, "WArc");
    ...
    assert_eq!(resolved.model_resref, "warc_b_053");
    assert_eq!(resolved.icon_resref, "iwarc_b_053");
...
#[test]
fn donor_clone_api_rejects_the_reserved_standalone_hextech_identity() {
...
fn custom_weapon_append_rejects_any_physical_index_other_than_requested_output() {
    ...
    request.label = "legacy_arc_cannon".to_owned();
    request.item_class = "WArc".to_owned();
    let error = append_item_custom_weapon_baseitem_v2(&source, &request).unwrap_err();
