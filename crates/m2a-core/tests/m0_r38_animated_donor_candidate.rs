...
use m2a_core::{
    animated_donor_candidate::{
        M0_R38_ACCEPTED_R37_PROOF_SHA256, M0_R38_APPEARANCE_ROW, M0_R38_AREA_RESREF,
        M0_R38_HAK_RESREF, M0_R38_MODEL_RESREF, M0_R38_MODEL_SHA256, M0_R38_MODULE_RESREF,
...
fn exact_m0_r38_historical_candidate_fails_closed_after_r45_skin_bind_upgrade() {
    assert_eq!(
    ...
    let artifact = match build_m0_r38_animated_donor_candidate_v1(&source, &donor, &appearance) {
        Ok(artifact) => artifact,
        Err(error) => {
            assert_eq!(error.code, "M2A-R38-RETARGET-CONTRACT");
            assert!(error.message.contains(M0_R38_MODEL_SHA256));
            assert!(
                error
                    .message
    ...
    assert_eq!(
        artifact.contract.admitted_by_r37_proof_sha256,
        M0_R38_ACCEPTED_R37_PROOF_SHA256
    );
    assert_eq!(artifact.contract.model.resref, M0_R38_MODEL_RESREF);
    assert_eq!(artifact.contract.model.sha256, M0_R38_MODEL_SHA256);
    ...
    let hak = ErfArchive::parse(&artifact.hak).expect("r38 HAK");
    assert_eq!(hak.resources().len(), 3);
    assert_eq!(hak.find(M0_R38_MODEL_RESREF, 2002).unwrap(), artifact.model);
    assert_eq!(
        hak.find(M0_R38_TEXTURE_RESREF, 3).unwrap(),
        artifact.texture
    );
    assert_eq!(
    ...
    let scene = inspect_binary_creature_multi_fixture_module_v1(&artifact.module).expect("r38 MOD");
    assert_eq!(scene.module_resref, M0_R38_MODULE_RESREF);
    assert_eq!(scene.area_resref, M0_R38_AREA_RESREF);
    assert_eq!(scene.ordered_hak_resrefs, [M0_R38_HAK_RESREF]);
    assert_eq!(scene.fixtures.len(), 1);
    assert_eq!(scene.fixtures[0].appearance_row, M0_R38_APPEARANCE_ROW);

    let mut wrong_source = source.clone();
    *wrong_source.last_mut().expect("source byte") ^= 1;
    let error = build_m0_r38_animated_donor_candidate_v1(&wrong_source, &donor, &appearance)
        .expect_err("source identity attack");
    assert_eq!(error.code, "M2A-R38-INPUT-IDENTITY");

    let mut wrong_contract = artifact.contract.clone();
    wrong_contract.model.sha256 = "0".repeat(64);
    let error = verify_m0_r38_animated_donor_candidate_v1(
        &wrong_contract,
        &source,
        &donor,
        &appearance,
    ...
    assert_eq!(error.code, "M2A-R38-REPLAY-MISMATCH");
}
