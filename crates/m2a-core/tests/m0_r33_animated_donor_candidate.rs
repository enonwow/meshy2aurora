...
fn exact_m0_r33_historical_candidate_fails_closed_after_r45_skin_bind_upgrade() {
    assert_eq!(
    ...
    let artifact = match build_m0_r33_animated_donor_candidate_v1(&source, &donor, &appearance) {
        Ok(artifact) => artifact,
        Err(error) => {
            assert_eq!(error.code, "M2A-R33-RETARGET-CONTRACT");
            assert!(error.message.contains(M0_R33_MODEL_SHA256));
            assert!(
                error
                    .message
    ...
    assert!(
        String::from_utf8_lossy(&artifact.appearance_two_da)
            .contains(&M0_R33_APPEARANCE_LABEL.to_owned())
    );
